...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		// Worker-thread entry dispatch must run before any async work (i18n init):
		// the stats sync worker's buffering onmessage handler is installed in the
		// synchronous prefix of `runWorkerEntrypoint`, and Bun flushes the worker's
		// parked initial messages as soon as the entry module's top-level evaluation
