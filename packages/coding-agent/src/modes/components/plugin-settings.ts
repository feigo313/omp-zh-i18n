...
export class PluginListComponent extends Container {
	...
	#renderItem(entry: PluginListEntry): SelectItem {
		...
		if (entry.kind === "npm") {
			...
			if (featureCount > 0) {
				details += ` ${theme.sep.dot} ${enabledCount}/${featureCount} ${interceptUIString("ui.plugins.features", "features")}`;
			}
		...
		if (summary.shadowedBy) {
			details += ` ${theme.sep.dot} ${interceptUIString("ui.plugins.shadowedBy", "shadowed by")} ${summary.shadowedBy}`;
		}
...
export class PluginDetailComponent extends Container {
	...
	async #rebuild(): Promise<void> {
		...
		items.push({
			...
			description: interceptUIString("ui.plugins.enableOrDisable", "Enable or disable this plugin"),
		...
		if (manifest.features && Object.keys(manifest.features).length > 0) {
			...
			for (const [featName, feat] of Object.entries(manifest.features)) {
				...
				items.push({
					...
					label: `  ${featName}`,
					description:
						feat.description || interceptUIString("ui.plugins.enableFeature", `Enable ${featName} feature`),
...
class ConfigInputSubmenu extends Container {
	...
	constructor(
		...
		// Type hint
		let hint = `${interceptUIString("ui.plugins.typeHint", "Type: ")}${schema.type}`;
