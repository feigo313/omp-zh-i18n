...
export class SettingsSelectorComponent implements Component {
	...
	/** Last rendered language, used to rebuild translated settings after a switch. */
	#lastLanguage = "";
	...
	#buildItemsForDefs(defs: SettingDef[]): SettingItem[] {
		...
		for (const def of defs) {
			...
			if (def.group && def.group !== lastGroup) {
				const translatedGroup = interceptGroupLabel(def.tab, def.group);
				items.push({ id: `__heading:${def.group}`, label: translatedGroup, currentValue: "", heading: true });
				lastGroup = def.group;
			}
			items.push(item);
		}
