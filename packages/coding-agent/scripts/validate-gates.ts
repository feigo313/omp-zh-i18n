...
const categories = ["shell","files","memory","appearance","tasks","providers","model","interaction","context","tools"];
let allPass = true;
...
for (const cat of categories) {
  ...
  if (!match) { console.log(`FAIL settings-${cat}: EN=${enK.length} ZH=${zhK.length}`); allPass = false; }
}
...
if (!cmdMatch) { console.log(`FAIL commands: EN=${enCmd.length} ZH=${zhCmd.length}`); allPass = false; }
