...
const zhTranslations: Record<string, { label: string; description: string }> = {
  ...
  "tui.scrollbackRebuild": {
    label: "重写回滚缓冲区",
    description: "当代码块的最终形式替换其活体预览时，擦除并重放终端回滚缓冲区。关闭时（默认），旧的预览副本会留在历史记录中，最终内容会追加在下方。",
  },
  "display.collapseCompacted": {
    label: "折叠已压缩的历史",
    description: "在实时对话记录中，将压缩前的历史折叠到摘要分隔线后面；禁用以在每次压缩点处使用分隔线保持完整对话记录。",
  },
  "tui.imeSafeCursor": {
    label: "IME 安全提示布局",
  ...
  "prewalk.enabled": {
    label: "启用预走查",
    description: "从当前活跃模型开始，然后在计划任务的待办列表存在后的第一次编辑/写入时切换到快速/廉价模型（默认为 'smol' 角色）。强模型负责计划、提交待办并开始实现，然后移交给 'smol' 角色。",
  },
  "tier.openai": {
    label: "服务层级 — OpenAI",
    description: "OpenAI / OpenAI-Codex 请求以及通过 OpenRouter 路由的 OpenAI 系列模型的处理层级（none = 不发送）。作为 service_tier 发送。",
  },
  "tier.anthropic": {
    label: "服务层级 — Anthropic",
    description: "Claude 请求的处理层级。priority 在受支持的直接 Anthropic 模型上启用快速模式（speed: fast）；在 Bedrock/Vertex Claude 和通过 OpenRouter 时忽略。",
  },
  "tier.google": {
    label: "服务层级 — Google",
    description: "Gemini（Google AI Studio + Vertex）请求以及通过 OpenRouter 路由的 Google 系列模型的处理层级（none = 不发送）。作为顶层 serviceTier 字段发送。",
  },
  "tier.subagent": {
    label: "服务层级 — 子代理",
    description: "为生成的任务/评估子代理设置服务层级。继承 = 匹配主代理的实时每族层级（跟踪 /fast）；选择一个值以应用到子代理模型所属的族。",
  },
  "tier.advisor": {
    label: "服务层级 — 顾问",
    description: "顾问模型的服务层级。无 = 标准处理；继承 = 匹配主代理的实时每族层级；选择一个值以应用到顾问模型的族。",
  },
  ...
  "tools.xdev": {
    label: "xd:// 工具",
    description: "将很少使用的（可发现的）工具挂载到 xd:// 设备 URL 下，通过读/写驱动而不是在每个请求上发送其 schema。禁用以将所有启用的工具暴露为顶级。",
  },
  ...
  "task.prewalk": {
    label: "通用任务预走查",
    description: "为内置通用 task 子代理启用预走查：它从其解析的模型开始，计划并开始实现，然后在第一次编辑/写入时移交给 smol 角色。每个代理的覆盖（task.agentPrewalk，通过 /agents 中的 P 切换）和用户代理 prewalk 前置内容无论此开关如何都会生效。",
  },
  ...
  "speech.enhanced": {
    label: "增强语音重写",
    description: "在合成之前，使用 tiny/smol 模型将助手输出重写为自然的口语化散文（描述代码，删除链接和 markdown）。失败时回退到机械清理。",
  },
  "exa.enabled": {
    label: "Exa",
  ...
  "providers.anthropic.serverSideFallback": {
    ...
    description: "当 Claude Fable 5 / Mythos 5 请求被 Anthropic 的安全分类器阻止时，在 Claude Opus 4.8 服务端重试（Anthropic server-side-fallback-2026-06-01 beta）。选择启用 — 保留此选项关闭以保持每个请求的回退前行为。",
  },
  ...
  "providers.maxInFlightRequests": {
    label: "最大并发请求",
    description: "每个提供商 ID（例如 \"openai\" 或 \"anthropic\"）的最大并发 LLM 请求数，与此配置根目录共享的本地 OMP 进程之间共享。省略的提供商无限制。",
  },
};
...
for (const key of phantomKeys) {
  ...
  // Also check en-settings-full.json
  const fullData = loadJson("en-settings-full.json");
  // phantom keys in full don't have settings. prefix - skip for now
}
...
const whitelist = new Set([
  "APFS", "ZFS", "btrfs", "ProjFS", "Overlayfs", "Reflink", "Unicode", "ASCII",
  "MCP", "LSP", "GitHub", "OpenAI", "Exa", "Exa Websets",
  "Bash", "Shell", "GLM", "Hermes", "Kimi", "Anthropic", "DeepSeek",
  "Gemini", "Gemma", "Harmony", "Qwen3", "MiniMax", "Fireworks",
  "Agent", "Git", "Mnemopi", "TTSR", "Native", "XML", "AGENTS.md",
]);
...
writeFileSync(join(import.meta.dir, "..", "..", "..", "docs", "local", "i18n-missing-fix-summary.md"), summaryLines.join("\n") + "\n");
