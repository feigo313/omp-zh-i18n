...
function parseWithDupes(json: string): { obj: Record<string, string>; dupes: Map<string, string[]> } {
  ...
  let match;
  while ((match = kvPattern.exec(clean)) !== null) {
    const key = match[1];
    const val = match[2];
    keys.push(key);
    values.push(val);
...
const footerKeys: Record<string, string> = {
  ...
  "ui.settings.footer.defaultWithSections": "Enter/Space to change · Tab to jump sections · ←/→ to switch tabs · Type to search · Esc to close",
