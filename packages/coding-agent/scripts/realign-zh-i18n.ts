...
function parseJsonDetectDupes(filePath: string): Record<string, string> {
	...
	let match: RegExpExecArray | null = keyPattern.exec(clean);
	while (match !== null) {
		const keyStr = match[0].slice(0, -1).trim();
		if (keyStr.startsWith('"')) {
			rawKeys.push(JSON.parse(keyStr));
		}
		match = keyPattern.exec(clean);
	}
...
function cmdReport() {
	...
	// Settings report
	const settingsReport: Record<
		string,
		{ en: number; zh: number; missing: string[]; extra: string[]; misplaced: string[]; orphan: string[] }
	> = {};
	let totalEn = 0,
		totalZh = 0,
		totalMissing = 0,
	...
	console.log(
		`${"TOTAL".padEnd(15)} ${String(totalEn).padStart(4)} ${String(totalZh).padStart(4)} ${String(totalMissing).padStart(5)} ${String(totalExtra).padStart(6)}`,
	);
...
function cmdSettingsApply() {
	...
	for (const cat of CATEGORIES) {
		...
		if (salvaged.length > 0)
			console.log(`  salvaged: ${salvaged.slice(0, 10).join(", ")}${salvaged.length > 10 ? "..." : ""}`);
...
function cmdCommandsApply() {
	...
	console.log(
		`[commands] EN:${enKeys.length} ZH:${newKeys.length} archive:${extras.length} salvaged:${salvaged.length} empty:${emptyQueue.length} keys_match:${keyMatch}`,
	);
