...
let match;
while ((match = keyPattern.exec(schema)) !== null) {
  const p = match[1];
  ...
  while (j < schema.length) {
    if (schema[j] === "{") depth++;
    else if (schema[j] === "}") { depth--; if (depth === 0) break; }
    j++;
  }
  ...
  entries.push({ path: p, label: labelMatch[1].trim(), tab });
}
...
const whitelist = new Set([
  ...
  "MCP","LSP","GitHub","OpenAI","Exa","Exa Websets","Bash","Shell",
  "GLM","Hermes","Kimi","Anthropic","DeepSeek","Gemini","Gemma","Harmony",
  "Qwen3","MiniMax","Fireworks","Agent","Git","Mnemopi","TTSR","Native","XML","AGENTS.md",
]);
